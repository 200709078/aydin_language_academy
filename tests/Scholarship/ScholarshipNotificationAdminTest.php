...
class ScholarshipNotificationAdminTest extends ScholarshipTestCase
{
    ...
    public function test_delivery_requires_published_data_and_keeps_sent_separate_from_contact(): void
    {
        ...
        self::assertStringContainsString('Not: Girilmedi', ScholarshipNotification::query()->where('phase', 'result')->sole()->body);
        self::assertStringContainsString('Boş Sayısı: Girilmedi', ScholarshipNotification::query()->where('phase', 'result')->sole()->body);
        $this->applications->updateResult($this->admin, $application->id, ['score' => 80, 'correct_count' => 35, 'wrong_count' => 0, 'blank_count' => 5]);
        self::assertSame('queued', $send('result'));
        $this->applications->updateResult($this->admin, $application->id, ['blank_count' => 4]);
        self::assertSame('queued', $send('result'));
        self::assertCount(4, Mail::mailer()->getSymfonyTransport()->messages());
        $body = ScholarshipNotification::query()->where('phase', 'result')->latest('id')->first()->body;
        self::assertStringContainsString('Yanlış Sayısı: 0', $body);
        self::assertStringContainsString('Boş Sayısı: 4', $body);
