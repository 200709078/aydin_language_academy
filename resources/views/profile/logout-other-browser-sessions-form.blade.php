<x-action-section>
    ...
    <x-slot name="description">
        {{ __('dictt.browsersessiondesc') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('dictt.browsersessionnecessary') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('dictt.thisdevice') }}</span>
                                    @else
                                        {{ __('dictt.lastactivity') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-dialog-modal wire:model.live="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('dictt.logoutbrowsersessionplease') }}
