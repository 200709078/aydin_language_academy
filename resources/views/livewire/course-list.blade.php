    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
                <h5 class="card-title mb-1">{{ __('dictt.courses_title') }}</h5>
                <a href="{{ route('course_create') }}" class="btn btn-sm btn-primary">
                    <i class="fa fa-plus"></i> {{ __('dictt.addnewcourse') }}</a>
            </div>
            <table class="table table-striped table-sm">
                <thead>
                    <tr>
                        <th scope="col">{{ __('dictt.title') }} (tr)</th>
                        <th scope="col">{{ __('dictt.title') }} (en)</th>
                        <th scope="col">{{ __('dictt.slogan') }} (tr)</th>
                        <th scope="col">{{ __('dictt.slogan') }} (en)</th>
                        <th scope="col">{{ __('dictt.description') }} (tr)</th>
                        <th scope="col">{{ __('dictt.description') }} (en)</th>
                ...
                <tbody>
                    @foreach($courses as $course)
                        <tr class="align-middle">
                            <th class="col-md-3" scope="row">{{Str::of($course->title_tr)->before(' ') . ' ...' }}</th>
                            <th class="col-md-3" scope="row">{{Str::of($course->title_en)->before(' ') . ' ...' }}</th>
                            <th class="col-md-3" scope="row">{{Str::of($course->slogan_tr)->before(' ') . ' ...' }}</th>
                            <th class="col-md-3" scope="row">{{Str::of($course->slogan_en)->before(' ') . ' ...' }}</th>
                            <th class="col-md-3" scope="row">{{Str::of($course->description_tr)->before(' ') . ' ...' }}
                            </th>
                            <th class="col-md-3" scope="row">{{Str::of($course->description_en)->before(' ') . ' ...' }}
                            </th>
                            <th class="col-md-3" scope="row">
                                @if($course->image)
                                    <a href="{{ asset('photos/' . $course->image) }}" target="_blank">
                                        <img class="img-fluid rounded align-self-end"
                                            src="{{ asset('photos/' . $course->image) }}" style="width:120px"
                            ...
                            <td>
                                <div class="flex gap-1">
                                    <a href="{{ route('course_edit', $course->id) }}" class="btn btn-sm btn-primary"
                                        title="{{ __('dictt.edit') }}">
                                        <i class="fa fa-pen w-4"></i>
                                    </a>
                                    <!--  -->
                                    <button wire:click="confirmDelete({{ $course->id }})" class="btn btn-sm btn-danger"
