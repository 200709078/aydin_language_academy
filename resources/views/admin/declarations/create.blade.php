<x-app-layout>
    <x-slot name="header">CREATE DECLARATIONS FOR {{Str::upper(Str::limit($theme->name, 20))}}</x-slot>
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">
            ...
            <form method="POST" action="{{ route('declaration_store', $theme->id)}}" enctype="multipart/form-data">
                ...
                <div class="form-group mt-2">
                    <button type="submit" class="btn btn-success btn-sm btn-block">DECLARATION CREATE</button>
                </div>
            </form>
        </div>
    </div>
